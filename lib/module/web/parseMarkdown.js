...
function initializeParser() {
  if (!parserPromise) {
    parserPromise = import('./wasm/md4c').then(module => module.default()).then(wasmModule => wasmModule.cwrap('parseMarkdown', 'string', ['string', 'number', 'number', 'number', 'number', 'number', 'number'])).catch(error => {
      parserPromise = null;
      throw error;
    });
  }
...
export async function parseMarkdown(markdown, {
  ...
  const result = JSON.parse(parse(markdown, underline ? 1 : 0, latexMath ? 1 : 0, superscript ? 1 : 0, subscript ? 1 : 0, highlight ? 1 : 0, hardSoftBreaks ? 1 : 0));
  if (!isASTNode(result)) {
