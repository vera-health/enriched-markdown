...
const defaultHeadingInternal = key => {
...
const normalizeHeadingStyle = (key, style) => {
...
let cachedInput;
let cachedResult;
function normalizeInputLinkStyle(style, fallback) {
  return {
    color: normalizeColor(style?.color) ?? fallback.color,
    underline: style?.underline ?? fallback.underline,
    backgroundColor: normalizeColor(style?.backgroundColor) ?? fallback.backgroundColor
  };
}
export const normalizeMarkdownTextInputStyle = style => {
  ...
  const result = {
    ...
    link: linkBase,
    linkVariants: normalizeLinkVariantEntries(style.linkVariants).map(([pattern, override]) => ({
      pattern,
      color: normalizeColor(override.color) ?? linkBase.color,
      underline: override.underline ?? linkBase.underline,
      backgroundColor: normalizeColor(override.backgroundColor) ?? defaultInternal.link.backgroundColor
    })),
    spoiler: {
...
//# sourceMappingURL=normalizeMarkdownTextInputStyle.js.map